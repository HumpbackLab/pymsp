...
class MSPv1:
    ...
    def pack(self, message_id: int, payload: bytes = b"") -> bytes:
        ...
        if len(payload) > 255:
            raise MSPException(
                f"Payload too large for MSP v1: {len(payload)} bytes. Maximum is 255 bytes."
            )
    ...
    def unpack(self, raw_message: bytes) -> MSPFrame:
        ...
        if checksum != expected_checksum:
            raise MSPException(
                f"Checksum mismatch: expected {expected_checksum:#02x}, got {checksum:#02x}"
            )
...
class MSPv2:
    ...
    def unpack(self, raw_message: bytes) -> MSPFrame:
        ...
        if (
            len(raw_message) < 9
        ):  # Minimum length: header(3) + flags(1) + id(2) + size(2) + checksum(1)
